...
This is C code for a Wall-E robot, running on a Raspberry PI. He has a number of servo motors that are controlled
by a PCA9685 16 channel PWM controller via I2C connected to the PI. He has 2 DC motors for the tracks, and they are
controlled by a L298 Motor Bridge connected to a Pro Mini Arduino board. The Raspberry sends commands to the Arduino
board to control the track motors. The board also controls audio and battery monitoring. This was to off-load different
functions from the PI as much as possible, so the PI could focus on image processing (usb camera) and other important
operations.
...
void updateHead(int x, int y, int d) {
    if (x != 0 && y != 0) {
        ...
        s[BOTTOM_NECK].targetPos = constrain(t, s[BOTTOM_NECK].min, s[BOTTOM_NECK].max);
        s[UPPER_NECK].targetPos = constrain(y, s[UPPER_NECK].min, s[UPPER_NECK].max);

        s[TOP_HEAD].targetPos = mapRange(0,640,s[TOP_HEAD].min,s[TOP_HEAD].max,x);
    }
}
